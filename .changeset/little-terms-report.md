...
Add a new `selectBlock` command and update the `Mod-a` keybinding behavior.
...
The `defineBaseKeymap` extension now changes the behavior of `Ctrl-A` (Windows/Linux) and `Command-A` (Mac): the first press selects the current block that the cursor is in, and a second press selects the entire document. This is similar to the behavior of Notion.
