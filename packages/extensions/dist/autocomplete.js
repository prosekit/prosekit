...
import { OBJECT_REPLACEMENT_CHARACTER, defineFacet, defineFacetPayload, isTextSelection, pluginFacet } from "@prosekit/core";
...
function handleTransaction(tr, prevValue, oldState, newState, getRules) {
	...
	if (!meta) {
		...
		if (prevMatching.rule.followCursor && !tr.docChanged && !tr.getMeta("pointer") && isTextSelection(newState.selection) && newState.selection.empty) return handleCursorMove(prevMatching, newState, getRules, ignores);
	...
	if (meta.type === "scan") {
		...
		const currMatching = matchRule(newState, getRules(), textBackward, textFrom, textTo, ignores);
		if (currMatching && prevMatching && prevMatching.from !== currMatching.from) ignores.push(prevMatching.from);
		return {
			matching: currMatching ?? null,
			ignores
		};
	}
	throw new Error(`Invalid transaction meta: ${meta}`);
}
function handleCursorMove(prevMatching, newState, getRules, ignores) {
	const head = newState.selection.head;
	if (head > prevMatching.from && head - prevMatching.from <= MAX_MATCH && newState.selection.$head.sameParent(newState.doc.resolve(prevMatching.from))) {
		const text = getTextBetween(newState.doc, prevMatching.from, head);
		return {
			matching: matchRule(newState, getRules(), text, prevMatching.from, head, ignores) ?? null,
			ignores
		};
	}
	return {
...
*/
function triggerAutocomplete(tr) {
...
export { AutocompleteRule, defineAutocomplete, triggerAutocomplete };
