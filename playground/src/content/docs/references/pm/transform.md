...
```
new Mapping(maps?: StepMap[], from?: number, to?: number): Mapping
```
...
```ts
function dropPoint(doc: ProseMirrorNode, pos: number, slice: Slice): number | null
```
...
```ts
function findWrapping(range: NodeRange, nodeType: NodeType, attrs?: null | Attrs, innerRange?: NodeRange): { attrs: Attrs | null; type: NodeType }[] | null
```
...
```ts
function insertPoint(doc: ProseMirrorNode, pos: number, nodeType: NodeType): number | null
```
...
```ts
function joinPoint(doc: ProseMirrorNode, pos: number, dir?: number): number | undefined
```
...
```ts
function replaceStep(doc: ProseMirrorNode, from: number, to?: number, slice?: Slice): Step | null
```
