...
- `"Ctrl-Shift-a"`: ❌ When Shift is held, pressing <Keyboard key="A" /> will trigger an event with [`KeyboardEvent.key`] property set to `"A"`, not `"a"`.
...
When pressing <Keyboard key="Mod+B" />, only the handler in the `extensionB` will be called.
