/**
 * This file is auto-generated by the "@aria-ui/cli" tool. Do not edit this file directly.
 */

import TableHandleDropIndicatorComponent from "./table-handle-drop-indicator.gen.svelte";
import { type TableHandleDropIndicatorProps as TableHandleDropIndicatorElementProps } from "@prosekit/web/table-handle";
import type { Component, Snippet } from "svelte";

/**
 * Props for the {@link TableHandleDropIndicator} Svelte component.
 *
 * @public
 */
export interface TableHandleDropIndicatorProps {
  /**
   * @default null
   * @hidden
   */
  editor?: TableHandleDropIndicatorElementProps["editor"];
  children?: Snippet;
  [key: string]: unknown;
}

/**
 * A Svelte component that renders an `prosekit-table-handle-drop-indicator` custom element.
 *
 * @public
 */
export const TableHandleDropIndicator: Component<TableHandleDropIndicatorProps> =
  TableHandleDropIndicatorComponent as unknown as Component<TableHandleDropIndicatorProps>;
