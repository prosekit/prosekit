...
describe('hasCodeBlockPreviewHiddenDecoration', () => {
  ...
  it('returns true when a decoration has the HIDE_CODE_BLOCK_PREVIEW spec', () => {
    const deco = Decoration.node(0, 1, {}, { [HIDE_CODE_BLOCK_PREVIEW]: true })
  ...
  it('returns true when one of multiple decorations has the HIDE spec', () => {
    ...
    const deco2 = Decoration.node(2, 3, {}, { [HIDE_CODE_BLOCK_PREVIEW]: true })
...
describe('defineCodeBlockPreviewDecorations', () => {
  it('adds hide-preview decoration when cursor is inside a code block', () => {
    ...
    expect(someDecorationHasHideSpec(found)).toBe(true)
  })
  ...
  it('does not add decorations when cursor is outside a code block', () => {
    ...
    const found = getCodeBlockPreviewDecorations(editor.view.state).find()
    expect(someDecorationHasHideSpec(found)).toBe(false)
  })
...
/**
 * Check if any decoration in the array has the HIDE_CODE_BLOCK_PREVIEW spec.
 */
function someDecorationHasHideSpec(decorations: readonly Decoration[]): boolean {
 ...
 */
function getCodeBlockPreviewDecorations(state: EditorState): DecorationSet {
  const plugin = codeBlockPreviewDecorationsPluginKey.get(state)
  const fn = plugin?.props.decorations
  if (plugin && typeof fn === 'function') {
    return (fn.call(plugin, state) as DecorationSet | null | undefined) ?? DecorationSet.empty
  }
