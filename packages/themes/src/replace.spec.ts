...
describe('replaceThemesWith', () => {
  it('can replace class names', () => {
    ...
    expect(replaceThemesWith(code, Themes)).toMatchInlineSnapshot(`
      "const App = () => (
        <div class='ProseMirror box-border min-h-full px-[max(4rem,_calc(50%-20rem))] py-8 outline-none outline-0 [&_span[data-mention="user"]]:text-blue-500 [&_span[data-mention="tag"]]:text-violet-500 [&_pre]:border [&_pre]:border-zinc-200 dark:[&_pre]:border-zinc-800 [&_pre]:border-solid'>
