...
```ts
const widget: (pos: number, toDOM: WidgetConstructor, spec?: { destroy?: (node: Node) => void; ignoreSelection?: boolean; key?: string; marks?: readonly Mark[]; side?: number; stopEvent?: (event: Event) => boolean; [key]: any }) => Decoration
```
...
```ts
const someProp: ((<PropName extends keyof EditorProps, Result>(propName: PropName, f: (value: NonNullable<EditorProps[PropName]>) => Result) => undefined | Result) | (<PropName extends keyof EditorProps>(propName: PropName) => undefined | NonNullable<EditorProps[PropName]>))
```
...
`plugins?: readonly ProseMirrorPlugin[]`
