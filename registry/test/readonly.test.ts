...
testStory('readonly', () => {
  it('readonly', async () => {
    ...
    await expect.element(editor).not.toMatchTextContent('foo')
    await expect.element(editor).not.toMatchTextContent('bar')
    await expect.element(editor).not.toMatchTextContent('baz')

    await editableButton.click()
    await focusEditor()
    await userEvent.type(editor, 'foo')
    await expect.element(editor).toMatchTextContent('foo')
    await expect.element(editor).not.toMatchTextContent('bar')
    await expect.element(editor).not.toMatchTextContent('baz')
    ...
    await expect.element(editor).toMatchTextContent('foo')
    await expect.element(editor).not.toMatchTextContent('bar')
    await expect.element(editor).toMatchTextContent('baz')
  })
})
