...
* ```
*/
function findMarkRange($pos, predicate) {
...
function getMarkRange($pos, type, attrs) {
	...
	return findMarkRange($pos, attrs ? (mark) => mark.type.name === markTypeName && isSubset(attrs, mark.attrs) : (mark) => mark.type.name === markTypeName);
}
/**
		...
		const register = (input) => {
			...
			for (const args of input) switch (args[0]) {
				...
				case "update":
					...
					break;
				case "unmount": unmountHandlers.push(args[1]);
			}
		};
...
* @internal
*/
const isApple = platform ? /Mac|iP(?:hone|[ao]d)/.test(platform) : false;
/**
...
* @internal
*/
const isWebKit = vendor ? /Apple Computer/.test(vendor) : false;
/**
