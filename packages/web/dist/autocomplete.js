...
import { defineDOMEventHandler, defineKeymap, union, withPriority } from "@prosekit/core";
...
function setupAutocompletePopup(host, _props) {
	...
	const listboxProps = createPopupListboxProps(filter, query, eventTarget, {
		get: getDisabled,
		set: () => {}
	});
...
function useKeyboardBridge(host, getEditor, getOpen, target) {
	...
	const compositionEnd = defineDOMEventHandler("compositionend", (view, event) => {
		compostionEndedAt = event.timeStamp;
		return false;
	});
	const keyDown = defineDOMEventHandler("keydown", (view, event) => {
		if (view.composing || event.defaultPrevented || !getOpen() || !EVENT_KEYS.includes(event.key) || compostionEndedAt && event.timeStamp - compostionEndedAt < 50) return false;
		target.dispatchEvent(event);
		return event.defaultPrevented;
	});
	const extension = union(keyDown, compositionEnd);
	useEditorExtension(host, getEditor, withPriority(extension, 4));
}
function useAutocompleteExtension(host, getEditor, getRegex, getAnchor, reference, handlers, setQuery, getQueryBuilder, getFollowCursor, requestOpenChange) {
	useEffect(host, () => {
		...
		const rule = createAutocompleteRule(regex, getAnchor, reference, handlers, setQuery, getQueryBuilder, followCursor, requestOpenChange);
...
function useEscapeKeydown(host, getEditor, handler) {
	const extension = withPriority(defineKeymap({ Escape: handler }), 4);
