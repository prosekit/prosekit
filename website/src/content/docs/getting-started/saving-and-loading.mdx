...
ProseKit provides utility functions for converting between plain JSON object, HTML string, and ProseMirror [Node](https://prosekit.dev/references/pm/model/#prose-mirror-node).

|             | to `JSON`        | to `HTML`        | to `Node`        |
| ----------- | ---------------- | ---------------- | ---------------- |
| from `JSON` |                  | [`htmlFromJSON`] | [`nodeFromJSON`] |
| from `HTML` | [`jsonFromHTML`] |                  | [`nodeFromHTML`] |
| from `Node` | [`jsonFromNode`] | [`htmlFromNode`] |                  |
