...
testStory('save-json', () => {
  it('save-json', async () => {
    ...
    await expect.poll(() => savedContent.nth(0).element().textContent ?? '').toContain(`"text":"Foo"`)
    ...
    await expect.poll(() => savedContent.nth(0).element().textContent ?? '').toContain(`"text":"Foo"`)
    await expect.poll(() => savedContent.nth(1).element().textContent ?? '').toContain(`"text":"FooBar"`)
