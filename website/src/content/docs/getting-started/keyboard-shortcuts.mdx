...
import KeyboardShortcut from '@/components/ui/keyboard-shortcut.astro';
...
- `"Mod"`: Maps to <KeyboardShortcut key="Command" /> on macOS and <KeyboardShortcut key="Ctrl" /> on other platforms
- `"Ctrl"`: <KeyboardShortcut key="Ctrl" /> key
- `"Alt"`: <KeyboardShortcut key="Alt" /> key (<KeyboardShortcut key="Option" /> on macOS)
- `"Shift"`: <KeyboardShortcut key="Shift" /> key
- `"Meta"`: <KeyboardShortcut key="Command" /> key on macOS, <KeyboardShortcut key="Windows" /> key on other platforms
...
- `"Enter"`: ✅ A keyboard shortcut that triggers when <KeyboardShortcut key="Enter" /> is pressed. No modifier keys are required.
- `"Shift-Enter"`: ✅ A keyboard shortcut that triggers when <KeyboardShortcut key="Shift+Enter" /> is pressed.
- `"Shift-enter"`: ❌ Invalid because `"enter"` is not a valid value for [`KeyboardEvent.key`].
- `"Ctrl-a"`: ✅ Trigger when <KeyboardShortcut key="Ctrl+A" /> is pressed
- `"Ctrl-A"`: ✅ Trigger when <KeyboardShortcut key="Ctrl+Shift+A" /> is pressed.
