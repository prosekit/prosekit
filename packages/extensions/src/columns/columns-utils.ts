...
import type { ColumnAttrs, ColumnLayoutInfo, FindColumnGroupResult, FindColumnResult } from './columns-types.ts'
...
/**
 * Find the nearest ancestor `columnGroup` node for the given position.
 */
export function findParentColumnGroup($pos: ResolvedPos): FindColumnGroupResult | undefined {
  const found = findParentNode((node) => node.type.name === 'columnGroup', $pos)
  return found
}
...
export function findParentColumn($pos: ResolvedPos): FindColumnResult | undefined {
  ...
  if (containerDepth < 0 || $pos.node(containerDepth).type.name !== 'columnGroup') {
    return
  }
  return {
...
/**
 * Find a column inside a `columnGroup` container by child index.
 */
export function findColumnByIndex(
  ...
  if (!Number.isInteger(containerPos) || containerPos < 0 || containerPos > state.doc.content.size) return
  const container = state.doc.nodeAt(containerPos)
  if (!container || container.type.name !== 'columnGroup') return
...
export function getColumnCount(
  ...
  const container = state.doc.nodeAt(containerPos)
  return container?.type.name === 'columnGroup' ? container.childCount : 0
}
...
export function getColumnLayoutAtPos(
  ...
  const found = findParentColumnGroup($pos)
  if (!found) return
...
export function normalizeColumnWidths(
  ...
  if (total <= 0 || minColumnWidth * widths.length > TOTAL_COLUMN_WIDTH) {
...
/**
 * Measure the rendered CSS `gap` value from a column group element.
 */
export function measureColumnGroupGap(container: Element): number {
...
export function findColumnBoundaryAtCoords(
  ...
  const container = event.composedPath().find((el) => (el as HTMLElement).classList?.contains('prosekit-column-group')) as
    | HTMLElement
    | undefined
