...
<Select.Root type="single" onValueChange={(v) => (value = v)}>
  <Select.Trigger
    class="inline-flex h-12 w-[296px] select-none items-center rounded-lg border border-border outline-current bg-background px-[11px] text-sm transition-colors"
  ...
  <Select.Portal>
    <Select.Content
      class="z-50 max-h-96 w-[var(--bits-select-anchor-width)] min-w-[var(--bits-select-anchor-width)] select-none rounded-lg border border-muted bg-background p- shadow-lg outline-none data-[side=bottom]:translate-y-1 data-[side=left]:-translate-x-1 data-[side=right]:translate-x-1 data-[side=top]:-translate-y-1 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2"
      sideOffset={10}
    >
      <Select.Viewport class="p-1">
        {#each themes as theme, i (i + theme.value)}
          <Select.Item
            class="flex h-10 w-full select-none items-center rounded-md py-2 pl-5 pr-1.5 text-sm capitalize outline-none duration-75 data-[highlighted]:bg-muted data-[disabled]:opacity-50 scroll-m-1"
            value={theme.value}
            label={theme.label}
