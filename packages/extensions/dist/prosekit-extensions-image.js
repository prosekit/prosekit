...
* @public
*/
function uploadImage({ uploader, file, pos, replace = false, onError }) {
	return (state, dispatch, view) => {
		...
		if (replace && pos != null) {
			if (replaceExistingImageURL(state, pos, objectURL, dispatch)) return true;
		}
		return insertNode({
			type: "image",
			attrs: { src: objectURL },
...
function replaceExistingImageURL(state, pos, imageURL, dispatch) {
	const node = state.doc.nodeAt(pos);
	if (!node || node.type.name !== "image") return false;
	if (node.attrs.src === imageURL) return true;
	if (dispatch) {
		const tr = state.tr;
