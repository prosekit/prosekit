...
describe('replaceThemes', () => {
  it('can replace class names', () => {
    ...
    expect(replaceThemes(code)).toMatchInlineSnapshot(`
      "
      const App = () => (
        <div class='ProseMirror box-border min-h-full px-[max(40px,_calc(50%-330px))] py-[32px] outline-none outline-0 [&_span[data-mention="user"]]:text-blue-500 [&_span[data-mention="tag"]]:text-violet-500 [&_pre]:text-white [&_pre]:bg-zinc-800'>
