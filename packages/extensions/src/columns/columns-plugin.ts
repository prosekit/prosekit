...
import { applyColumnGroupMetaAction, type ColumnGroupMetaAction } from './columns-state.ts'
import type { ColumnAttrs, ColumnDragState, ColumnGroupRuntimeState } from './columns-types.ts'
import {
...
export const columnGroupPluginKey: PluginKey<ColumnGroupRuntimeState> = new PluginKey<ColumnGroupRuntimeState>('columnGroup')

/**
 * @internal
 */
export type ColumnGroupPluginExtension = PlainExtension

/**
 * Options for the column resize plugin.
 */
export interface ColumnGroupPluginOptions {
  /**
 ...
 */
export function getColumnGroupRuntimeState(state: EditorState): ColumnGroupRuntimeState | undefined {
  return columnGroupPluginKey.getState(state)
}
...
/**
 * Collect the column DOM elements inside a column group.
 */
 ...
 */
function snapshotColumnGroup(container: Element, leftIndex: number): {
  ...
  const gapPx = measureColumnGroupGap(container)
...
function applyColumnWidthPreview(
  ...
  if (container) {
    container.style.setProperty('--prosekit-column-group-count', String(columnEls.length))
  }
...
function startDrag(
  ...
  const onMove = (event: MouseEvent) => {
    ...
    window.requestAnimationFrame(() => {
      const state = getColumnGroupRuntimeState(view.state)
      if (!state?.dragging) {
        ticking = false
        return
      }
  ...
  const onEnd = (event: MouseEvent) => {
    ...
    tr.setMeta(columnGroupPluginKey, { type: 'stopDragging' } satisfies ColumnGroupMetaAction)
    view.dispatch(tr)
  }
  ...
  view.dispatch(
    view.state.tr.setMeta(
      columnGroupPluginKey,
      {
        ...
        pos: columnPos,
      } satisfies ColumnGroupMetaAction,
    ),
  )
  return false
}
...
function handleMouseDown(
  ...
  if (
    ...
    handleAddColumnClick(view, columnPos, defaultColumnWidth, maxColumns)
...
function handleAddColumnClick(
  ...
  tr.setMeta(columnGroupPluginKey, { type: 'setActiveHandle', pos: null } satisfies ColumnGroupMetaAction)
  view.dispatch(tr)

  // Run the add-column-after command.
  const command = createAddColumnAfter({ defaultColumnWidth, maxColumns })
...
function beginColumnDrag(
  ...
  if (!container?.classList.contains('prosekit-column-group')) return false
  ...
  const snap = snapshotColumnGroup(container, leftIndex)
...
function columnResizeDecorations(
  ...
  const runtime = getColumnGroupRuntimeState(state)
 ...
 */
export function defineColumnGroupPlugin(options: ColumnGroupPluginOptions = {}): ColumnGroupPluginExtension {
  ...
  return definePlugin(
    new Plugin<ColumnGroupRuntimeState>({
      key: columnGroupPluginKey,
      state: {
        init: (): ColumnGroupRuntimeState => ({
          activeHandle: null,
          dragging: null,
        }),
        apply(tr, prev) {
          const action = tr.getMeta(columnGroupPluginKey) as ColumnGroupMetaAction | undefined
          return applyColumnGroupMetaAction(prev, tr, action)
        },
      },
      props: {
        attributes: (state): Record<string, string> => {
          ...
          if (runtime?.activeHandle != null) {
            return { class: 'prosekit-column-group-resizing' }
          }
        ...
        handleDOMEvents: {
          ...
          mousedown: (view, event) => handleMouseDown(view, event, handleWidth, minColumnPercent, defaultColumnWidth, maxColumns),
        },
        decorations: (state) =>
