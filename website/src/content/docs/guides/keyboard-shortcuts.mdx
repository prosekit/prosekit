...
description: Bind editor commands to keyboard shortcuts with defineKeymap or framework-specific useKeymap hooks
sidebar:
  order: 30
---
...
- `"Shift-enter"`: ❌ Invalid because `"enter"` is not a valid [`KeyboardEvent.key`] value
- `"Ctrl-a"`: ✅ Triggers when <Keyboard key="Ctrl+A" /> is pressed
- `"Ctrl-A"`: ✅ Triggers when <Keyboard key="Ctrl+Shift+A" /> is pressed
- `"Ctrl-Shift-A"`: ❌ Invalid because `"Shift"` modifier is implicit with uppercase letters
- `"Ctrl-Shift-a"`: ❌ Invalid because when <Keyboard key="Shift" /> is held, [`KeyboardEvent.key`] is `"A"`, not `"a"`
