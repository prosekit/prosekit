...
function setupAutocompleteRoot(host, props) {
	...
	useAutocompleteExtension(host, getEditor, props.regex.get, getAnchor, reference, handlers, setQuery, props.queryBuilder.get, (open) => overlayStore.requestOpenChange(open));
}
const EVENT_KEYS = [
	"ArrowDown",
	"ArrowRight",
...
function useAutocompleteExtension(host, getEditor, getRegex, getAnchor, reference, handlers, setQuery, getQueryBuilder, requestOpenChange) {
	useEffect(host, () => {
		const editor = getEditor();
		const regex = getRegex();
		if (!editor || !regex) return;
		const extension = defineAutocomplete(createAutocompleteRule(regex, getAnchor, reference, handlers, setQuery, getQueryBuilder, requestOpenChange));
		return editor.use(extension);
	});
}
function createAutocompleteRule(regex, getAnchor, reference, handlers, setQuery, getQueryBuilder, requestOpenChange) {
