...
`get plugins(): readonly ProseMirrorPlugin[]`
...
```ts
const reconfigure: (config: { plugins?: readonly ProseMirrorPlugin[] }) => EditorState
```
...
```ts
const toJSON: (pluginFields?: { [propName]: ProseMirrorPlugin }) => any
```
...
```ts
const fromJSON: (config: { plugins?: readonly ProseMirrorPlugin[]; schema: Schema }, json: any, pluginFields?: { [propName]: ProseMirrorPlugin }) => EditorState
```
...
```ts
const getMeta: (key: string | ProseMirrorPlugin | PluginKey) => any
```
